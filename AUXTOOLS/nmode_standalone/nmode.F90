...
#  include "epot.h"
   ...
   if (ntrun == 1) then
      ...
      call mweight(x(mh),x(mf),rdummy,0,ns3,x(mamass),rdummy,idummy)
      ...
      if( ismem == 0 ) then    ! use the dsyev LAPACK routine
         ...
         write(6,'(a10,i8,a22,f8.0)') &
               '| lwork = ',lwork,'; best value would be ',x(mcscr)
         call writlw(lworki,int ( x(mcscr+4*ns3) ) )
      ...
      if (nvect > 0 .and. idecomp == 0) then
         call mweight(rdummy,rdummy,rdummy,2,ns3,x(mamass),x(mcvec),nvect)
         if (ivform == 0) then
...
! JK  Di+
         write(6,*) '...but code has been changed for this.'
!    Code added by Jacob Kongsted 2007 for new MM/PBSA entropy
! JK Di+ (idecomp variable was added) 
         nvecs =  3*natsys - 6
!        print*,"natsys,nvecs,ilevel,idecomp,mx,mamass,mcval,mh,ns3,t,patm"
!        print*,natsys,nvecs,ilevel,idecomp,mx,mamass,mcval,mh,ns3,t,patm
         call thermo(natsys,nvecs,ilevel,idecomp,x(mx),x(mamass),x(mcval), &
...
!    End of new code
      ...
      call mweight(x(mh),x(mf),x(1),0,nr3,x(mamass),rdummy,idummy)
