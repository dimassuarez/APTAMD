...
subroutine ephi(nphi,ip,jp,kp,lp,icp,pk,pn,phase,cn1,cn2,ntypes, &
   ...
   dimension ip(nphi),jp(nphi),kp(nphi),lp(nphi),icp(nphi), &
         pk(*),pn(*),phase(*)
!Di+ snb and see are now arrays for variable scaling factors
   dimension snb(*),see(*) 
...
!  scee = see
   ...
   do 200 iphi = 1,nphi
      ...
      l3 = lp(iphi)
      ic = icp(iphi)
!Di+ variable scaling factors
      scnb = snb(ic)
      scee = see(ic)
!     print*,iphi,ic,scee,scnb
!     if ( scnb .eq. 0.0 ) it means k3 < 0. This is well treated below
!     if ( scee .eq. 0.0 ) it means k3 < 0
!Di+
...
! Di+ this message is NOT an error and should not be printed
...
!Di+  /scee aded to the eelp calculation here for variable scaling
