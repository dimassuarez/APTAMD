...
#  include "sizes2.h"
   ...
   call ephi (nphih,     ix(miph),  ix(mjph), ix(mkph), &
         ...
         x(mx),     x(mf),     x(mh), &
         edihh,     enb14h,    eel14h, &
         x(mscnb),  x(mscee),      nphih,    epcnh, &      !Di+  scee and scnb replaced by the x() arrays
   ...
   call ephi (nphia,     ix(mipa),  ix(mjpa), ix(mkpa), &
         ...
         x(mx),     x(mf),     x(mh), &
         ediha,     enb14a,    eel14a, &
         x(mscnb), x(mscee),   nphia,    epcna, &          !Di+  scee and scnb replaced by the x() arrays
         idiel,     ndrv,      ix(mnbel), natsys)

   if(icons == 1) call xconst(econs,x(mx),x(mf),x(mh),ndrv, &
         x(mxref),x(mwref),natom)

! Di+ for consistency with the SANDER calc we also assign zero forces to the
! non-belly atoms. Note that this operation has no effect on the entropy
! values
   if (ibelly /= 0) call bellyf(natom,ix(mgroup),x(mf))
   !!
   !!     ----- put the belly force vector into the first part of array f
!  call getf(natom,ix(mgroup),x(mf)) 
   !! Di+ Not clear why this is done because there is no change like this for
   !! for coordinates. 
