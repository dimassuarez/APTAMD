...
where *selected file.pdb* would be your selected GaMD structure (only solute atoms).  A new edition step is then executed:
...
Using `do_struct.sh` and `do_mmpbsa.sh` the evolution of structural and/or energetic is monitored along the cMD simulations. Eventually, these descriptors may exhibit pronounced drifts and/or wide oscillations at the beginning of the cMD so that equilibrium properties and clustering analysis would be better evaluated using only the “fully-relaxed” part of the  trajectory.
...
The RMSD and RGYR values are calculated, but we skip now the INF ones (see  [input_struct_md.src](EXAMPLE_INPUT_FILES/input_struct_md.src) ). 

`do_struct 1DB6_model_MD/input_struct_md.src`

The resulting plot suggests a small structural transision during the cMD.

<img src="./Images/struct_md.png" width="800" height="450">
...
The PDB files for the snapshots are saved in 6.ANALYSIS/SNAPSHOTS. In the same directory, several files contain data characterizing the Na+···APT contacts. The `do_mmpbsa.sh` script will take coordinates from these snapshots to calculate MM-PBSA like energies (many options can be selected). 

`do_mmpbsa 1DB6_model_MD/input_mmpbsa.src`

SANDER/PBSA output files and other files with parsed data are saved in the 6.ANALYSIS/MMPBSA directory. Since different inner dielectric constants (default is 1), the MMPBSA files are actually written in subdirectories named as PDIE_1, PDIE_4, etc.  The most important files are those with the `.med` extension, which contain the mean values and statistical uncertainties of energy components.  The corresponding `.png` display the time evolution     
